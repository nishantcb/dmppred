...
parser.add_argument("-w","--winleng", type=int, choices =range(8,30), help="Window Length: 8 to 30 (scan mode only), by default 9")
...
if Job == 1:
    ...
    df1 = lenchk(dfseq)
    X = feature_gen(df1)
    mlres = model_run(X,'model/diabetes_model_150f.sav')
